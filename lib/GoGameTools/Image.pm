...
use GoGameTools::Class
  qw($image $board $node_for_markup $should_draw_ownership);
use Imager;
...
sub run ($self) {
    ...
    $self->draw_black_stone_at(@$_) for @black_stones;
    $self->draw_white_stone_at(@$_) for @white_stones;
    $self->draw_ownership;
    binmode STDOUT;
...
sub draw_white_stone_at ($self, $x, $y) {
...
sub draw_black_stone_at ($self, $x, $y) {
    my ($cx, $cy) = $self->get_coords_for_intersection($x, $y);
    $self->image->circle(
        color => $self->get_color('black_stone'),
        r     => STONE_SIZE,
...
sub draw_ownership ($self) {
    ...
    for my $x (1 .. 19) {
        for my $y (1 .. 19) {
            my $ownership = $ownership[ 19 * ($y - 1) + $x - 1 ];
...
dummy class that just runs a proof-of-concept
    ...
    perl -MGoGameTools::Image -e 'GoGameTools::Image->new->run' >out.png
